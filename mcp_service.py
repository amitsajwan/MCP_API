    ...
    def initialize(self):
        ...
        try:
            ...
            try:
                ...
                self._azure_client = loop.run_until_complete(create_azure_client())
                ...
                loop.run_until_complete(self._mcp_client.__aenter__())
                logger.info("✅ [MCP_SERVICE] Connected to MCP server")
                
                logger.info("🔄 [MCP_SERVICE] Loading tools...")
                self._tools = loop.run_until_complete(list_and_prepare_tools(self._mcp_client))
    ...
    def process_message(self, user_message: str, conversation_history: List[Dict] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            try:
                ...
                response = loop.run_until_complete(self._azure_client.chat.completions.create(
                ...
                if choice.finish_reason == "tool_calls":
                    ...
                    for i, tool_call in enumerate(choice.message.tool_calls, 1):
                        ...
                        try:
                            ...
                            raw_result = loop.run_until_complete(self._mcp_client.call_tool(tool_name, args))
