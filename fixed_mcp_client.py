#!/usr/bin/env python3
"""
MCP Client - Fixed HTTP Implementation
Fixed MCP client that properly connects to the HTTP MCP server endpoints.
"""
...
class MCPClient:
    ...
    def __init__(self, server_host: str = "localhost", server_port: int = None):
        """
        ...
        self.server_port = server_port or getattr(config, 'MCP_PORT', 9000)
        self.server_url = f"http://{server_host}:{self.server_port}"
        ...
        try:
            ...
            async with self.session.get(f"{self.server_url}/health") as resp:
                if resp.status == 200:
                    ...
                    logger.info(f"   API Specs: {data.get('api_specs', 0)}")
                    logger.info(f"   Tools: {data.get('tools', 0)}")
                    return
                else:
    ...
    async def call_tool(self, tool_name: str, arguments: Dict[str, Any] = None) -> Dict[str, Any]:
        ...
        try:
            ...
            async with self.session.post(f"{self.server_url}/call_tool", json=payload) as resp:
                if resp.status == 200:
...
Call tools when needed, then provide a helpful response based on the results."""
        ...
        try:
            ...
            response = self.openai_client.chat.completions.create(
                model=getattr(config, 'AZURE_OPENAI_DEPLOYMENT', 'gpt-4'),
                messages=[
...
async def main():
    """Example usage of MCP Client - HTTP connection."""
    print("MCP Client Test (HTTP)")
    print("=====================")
    print()
    print("🔌 Using HTTP connection to MCP server")
    print("📋 Make sure to start the MCP server with HTTP transport:")
    print("   python mcp_server.py --transport http --port 9000")
    print()
    
    # Create HTTP client
    client = MCPClient(server_host="localhost", server_port=9000)
    ...
    try:
        ...
        if tools:
            ...
            cred_result = await client.call_tool("set_credentials", {
                "username": "test_user",
            ...
            login_result = await client.perform_login()
            print("Login result:")
