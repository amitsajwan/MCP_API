#!/usr/bin/env python3
"""
FastMCP 2.0 Client Implementation
This is a FastMCP 2.0 client that uses the official FastMCP protocol with stdio transport.
"""
...
try:
    ...
    class DefaultConfig:
        LOG_LEVEL = "INFO"
        MCP_SERVER_SCRIPT = "mcp_server_fastmcp.py"
...
class FastMCPClientWrapper:
    """FastMCP 2.0 Client using the official FastMCP protocol with stdio transport"""
    ...
    async def list_tools(self) -> List[Dict[str, Any]]:
        """Get list of available tools from FastMCP server."""
    ...
    async def call_tool(self, tool_name: str, arguments: Dict[str, Any] = None) -> Dict[str, Any]:
        """Call a tool on the FastMCP server using FastMCP 2.0 protocol."""
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                logger.info(f"Calling FastMCP tool: {tool_name} with arguments: {arguments} (attempt {attempt + 1})")
                ...
                if attempt < self.max_retries - 1:
                    ...
                    logger.error(f"Error calling FastMCP tool {tool_name} after {self.max_retries} attempts: {e}")
    ...
    def _build_tools_description(self) -> str:
        ...
        for tool in self.available_tools:
            desc_parts = [f"🔧 {tool.get('name', 'Unknown')}"]
            desc_parts.append(f"   Description: {tool.get('description', 'No description')}")
    ...
    async def process_query(self, user_query: str) -> Dict[str, Any]:
        ...
        if not await self.connect():
            return {"status": "error", "message": "Failed to connect to FastMCP server"}
    ...
    async def _azure_4o_plan(self, user_query: str) -> List[ToolCall]:
        ...
        try:
            ...
            tool_calls = await azure_client.create_tool_plan(user_query, self.available_tools)
    ...
    def _simple_plan_fallback(self, user_query: str) -> List[ToolCall]:
        ...
        if any(keyword in query_lower for keyword in ["payment", "pending", "show", "list"]):
            ...
            for tool in self.available_tools:
                if "cash_api" in tool.get('name', '') and "getPayments" in tool.get('name', ''):
                    tool_calls.append(ToolCall(
        ...
        if any(keyword in query_lower for keyword in ["summary", "balance", "overview", "total"]):
            ...
            for tool in self.available_tools:
                if "cash_api" in tool.get('name', '') and "getCashSummary" in tool.get('name', ''):
                    tool_calls.append(ToolCall(
                        tool_name=tool.get('name'),
    ...
    async def perform_login(self) -> Dict[str, Any]:
        """Call the perform_login tool on the FastMCP server."""
        logger.info("Attempting to perform login via FastMCP server tool.")
        try:
...
async def main():
    """Demonstration of FastMCP 2.0 Client."""
    print("FastMCP 2.0 Client - Working Version")
    print("====================================")
    print()
